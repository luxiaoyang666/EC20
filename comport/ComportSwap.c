 ...
 *    Description:  This file communicate with Serial
 *                 
...
int comport_send(MyAttr *attr,char *sbuf,int sbuf_len)
{
    ...
    if(sbuf_len > attr->mSend_Len)  //指令长度实际长度大于单次发送的最大长度，则每次发送单次发送的最大长度
    {
        ptr = sbuf;
        end = sbuf + sbuf_len;
        ...
        do
        {
            if(attr->mSend_Len < (end - ptr))  //剩余长度大于单次发送的最大长度
            {
            ...
            {
                retval = write(attr->fd,ptr,(end - ptr));  //剩余长度可一次性发送
